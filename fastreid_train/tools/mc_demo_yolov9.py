...
def detect(save_img=False):
    ...
    # Load model
    model = DetectMultiBackend(weights, device=device, fp16=True)  # load FP32 model
    ...
    for path, img, im0s, vid_cap in tqdm(dataset, desc=f'tracking {opt.name}'):
        ...
        # Apply NMS
        pred = non_max_suppression(pred, opt.conf_thres, opt.iou_thres, classes=opt.classes, agnostic=opt.agnostic_nms, max_det=1000)
